...
//task 2
const multiply = (miltiplier, ...arguments) => arguments.map(argument => argument * miltiplier)
...
class Productclass {
    ...
    purchase(quantityToBuy) {
        if (quantityToBuy <= this.quantity) {
            ...
            console.log(`The purchase of ${quantityToBuy} units of ${this.name} has been completed`);
        } else {
            console.log(`There is not enough product ${this.name} in stock to purchase.`);
        }
    }
}
